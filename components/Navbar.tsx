...
import { MdNotificationsNone,MdChatBubble } from "react-icons/md";
...
const Navbar = (props) => {
  ...
  if (session) {
    if (session.user.name.toLowerCase() != "admin") {
      if (userDetails && userDetails.penalty && notifications.length == 0) {
        setNotifications([{ title: "Please pay your penalty" }]);
        setPenalty(true);
      }
    }
  }
    ...
    <div className="shadow-lg md:sticky top-0 z-40">
        ...
        <nav className="bg-white flex justify-between max-w-8xl sm:px-6 lg:px-[200px]">
          ...
          <div className="w-[25vw] flex">
            <input
              value={searchInput}
              type="text"
              placeholder={props.book ? props.book : "Search a Item"}
          ...
          <div className="flex items-center">
            <a href="/" target="_blank">
              <div className="flex flex-col items-center group">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-7 w-7 sm:mx-3  text-skin-darkBlue cursor-pointer hover:scale-110 transition-all"
                  viewBox="0 0 20 20"
                  fill="currentColor"
                >
                  <path
            ...
            <span className="hidden xl:block">
              {session.user.name.toLowerCase() != "admin" && (
                <Link href="/postBook">
                  <button
                    disabled={penalty}
                    title={penalty ? "Please submit your penalty" : ""}
                    onClick={() => {
                      setProgress(100);
                    }}
                    className="bg-skin-darkBlue text-white py-2 px-6 rounded-lg mx-4 font-bold transition-all disabled:bg-blue-800 disabled:cursor-not-allowed"
                  >
                    Sell
                  </button>
                </Link>
              ...
              {session.user.name.toLowerCase() === "admin" ? (
                ...
                <Menu>
                  <MenuButton
                    ...
                    className="bg-skin-darkBlue text-white py-2 px-6 rounded-lg mx-4 font-bold transition-all"
                  >
                    Items
                  </MenuButton>
                  <MenuList className="bg-white text-black px-2 py-2 rounded-md">
                    <Link href="/admin/pendingBooks">
                      <MenuItem className="p-1 hover:bg-skin-lightBlue transition-all rounded font-semibold text-center">
                        Pending Items
                      </MenuItem>
                    </Link>
                    <Link href="/issuedBooks">
                      <MenuItem className="p-1 hover:bg-skin-lightBlue transition-all rounded font-semibold text-center">
                        Issued Items
                      </MenuItem>
                    </Link>
                    <Link href="/borrowBooks">
                      <MenuItem className="p-1 hover:bg-skin-lightBlue transition-all rounded font-semibold text-center">
                        Borrow Items
                      </MenuItem>
                    </Link>
                    ...
                    <Link href="/admin/generateReports">
                      <MenuItem className="p-1 hover:bg-skin-lightBlue transition-all rounded font-semibold text-center">
                        Generate Report
                      </MenuItem>
                    </Link>
                 ...
                 <span className="hidden xl:block">
                  <Link href="/issuedBooks">
                    <button
                      ...
                      className="bg-lightRed  text-white  py-2 px-6 rounded-lg mx-4 font-bold transition-all disabled:bg-red-500 disabled:cursor-not-allowed"
                    >
                      Buy
                    </button>
                  </Link>
                  ...
                  <Menu>
                    <div className="relative">
                      <MenuButton
                        as={IconButton}
                        aria-label="Options"
                        icon={<MdNotificationsNone fontSize={28} />}
                        variant="outline"
                      />
                    ...
                    <MenuList className="bg-white shadow-md rounded-md w-96 mt-2">
            ...
            <Menu>
              ...
              <MenuList className=" bg-white rounded-lg p-2 w-36 shadow-lg">
                ...
                {session.user.name.toLowerCase() != "admin" && (
                  <Link href={"/returnBooks"} as={`/returnBooks`}>
                    <MenuItem
                      className="p-1 hover:bg-skin-lightBlue transition-all rounded font-semibold"
                      onClick={() => {
                        setProgress(40);
      ...
      {!session   && (
        <nav className="bg-white flex justify-between max-w-8xl sm:px-6 lg:px-[200px]">
          ...
          <div className="flex items-center">
             <a
              href="https://Qthriftdocs.super.site/"
              target="_blank"
            >
              <div className="flex flex-col items-center group">
                <svg
             ...
             <Link href="/auth/login">
              <button
                ...
                className="bg-skin-darkBlue cursor-pointer text-white shadow-lg py-2 px-6 rounded-lg mx-4 font-bold  inline-flex items-center gap-x-2"
              >
                <span>Sign In</span>
                <AiOutlineLogin />
              </button>
