...
import { sellBooks } from "../utils/sample-data";

const NewlyAddedHome = (props) => {
  const { templateString } = template;

  const [result, setResult] = useState(sellBooks || []);
      ...
      <div className="sm:hidden 2xl:grid 2xl:grid-cols-5 gap-1 w-11/12 m-auto md:hidden lg:hidden hidden mt-32 mb-6 pt-5">
        <div className="col-span-5  mb-12 px-[2vw]">
          ...
          <Link href="/browsebooks/newlyadded">
            <button
              className="float-right bg-lightRed text-white py-2 px-4 rounded-lg font-semibold text-xl hover:bg-skin-hoverBlue transition-all"
              onClick={() => {
                props.topLoader();
              }}
            >
