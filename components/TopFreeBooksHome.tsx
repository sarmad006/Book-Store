...
import { donateBooks } from "../utils/sample-data";


const TopFreeEduBook = ({ topLoader }) => {

  const [result, setResult]  = useState <string[]>(donateBooks)
  const [isFteched, setISFetched] = useState(false)
    ...
    <>
      ...
      <div className="sm:hidden 2xl:grid 2xl:grid-cols-5 gap-1 w-11/12 m-auto md:hidden lg:hidden hidden mt-32 mb-6">
        <div className="col-span-5  mb-12 px-[2vw]">
          ...
          <Link href="/">
            <button className="float-right bg-skin-darkBlue text-white py-2 px-4 rounded-lg font-semibold text-xl hover:bg-skin-hoverBlue transition-all" onClick={() => {topLoader()}}>
              View all
            </button>
          </Link>
        </div>
        ...
        {result && result.slice(0, 5).map((edubook) => {
          return (
      ...
      <div className="sm:hidden  grid grid-cols-1 my-14">
        ...
        <Link href="/browsefreebooks">
          <button className="float-right bg-skin-darkBlue text-white py-2 px-4 rounded-lg font-semibold text-xl hover:bg-skin-hoverBlue transition-all w-8/12 ml-auto mr-auto" onClick={() => {topLoader()}}>
            View all
          </button>
        </Link>
      </div>
